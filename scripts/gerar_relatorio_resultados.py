...
def main() -> int:
    ...
    if top_errors:
        executive_summary = [
            ...
            "- Os erros registrados indicam problema de massa/autorizacao quando LOAD_TEST_PROFESSOR_ID nao acessa os IDs usados no CSV.",
        ]
    report = [
        ...
        "",
        *executive_summary,
        "- Spring ficou consistentemente mais rapido nas leituras; Python deve ser avaliado sem custo de JWT nos planos atuais.",
        "",
        "## Por cenario",
        "",
        ...
        "## Leitura tecnica",
        "",
        "Os erros relevantes sao 403 Forbidden nos endpoints Python de leitura por ID, modulos, aulas e prova. Em modo sem JWT, isso normalmente indica que LOAD_TEST_PROFESSOR_ID nao tem acesso a todos os IDs de `python_read_ids.csv`.",
        "",
        "Os planos atuais nao devem gerar amostras de login Python. Se uma execucao ainda mostrar `Python Login`, ela foi feita com plano antigo ou arquivo JTL antigo.",
        "",
        "## Melhorias sugeridas no teste",
        "",
        "1. Manter `LOAD_TEST_MODE=true` no FastAPI durante as execucoes JMeter sem JWT.",
        "2. Validar se `LOAD_TEST_PROFESSOR_ID` consegue acessar os IDs de `python_read_ids.csv` antes de comparar performance.",
        "3. Separar dashboards de leitura, escrita, regressao e carga. Resultados antigos com login devem ficar identificados como historicos.",
